...
- Poor handwriting can make one specific digit or character genuinely ambiguous between two visually similar ones (e.g. a poorly-formed 3 that could be a 5, a 1 that could be a 7, a 6 that could be a 0) -- this is a reading problem, not a math problem, and must not be treated as a wrong answer. When you genuinely cannot tell which of two similar-looking characters was written, and one reading makes the student's own shown working and final answer internally consistent and mathematically correct while the other reading does not, resolve the ambiguity as the reading that is consistent with their own correct work -- do not mark it wrong based on the other, inconsistent reading. This applies ONLY to a real visual ambiguity on a single digit/character where the student's method is otherwise sound; it does NOT apply to an answer that is clearly and unambiguously written as something wrong, and it must never be used to excuse an actual calculation error or a flawed method -- when genuinely unsure whether something is an illegible-handwriting misread or a real mistake, use the redo-the-calculation-yourself step (see questionByQuestionCheck below) to check which reading the student's own working actually supports.
