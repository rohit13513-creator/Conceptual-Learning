...
interface RevisionSubjectConfig {
  ...
  // Extra context appended to generateRevisionPaper's system prompt for this subject only (see
  // that function). Needed for a subject that is a genuinely separate, non-standard course rather
  // than the regular NCERT syllabus -- the prompt's own chapter-purity guidance is framed entirely
  // around "don't blend in a DIFFERENT NCERT chapter", which never warns against blending in the
  // STANDARD course's content for a chapter that happens to share a name (e.g. Advanced Maths'
  // own "Coordinate Geometry" versus the basic quadrant-plotting chapter of the same name in
  // standard Class 9 Maths, or "Relations and Functions", which is normally Class 11 content).
...
const REVISION_SUBJECTS: readonly RevisionSubjectConfig[] = [
  ...
  { key: "science", dbPrefix: "science", label: "Science", classKeys: null },
  {
    ...
    curriculumNote: "This is the separate CBSE optional module \"Mathematics at Advanced Level\", not the standard NCERT Class 9 Maths course a student following this syllabus would also be studying separately. Some of its chapter names overlap with standard-course chapters that cover the same broad topic at a far more basic level (its own \"Coordinate Geometry\" chapter is not the same as the basic quadrant-plotting chapter of that name in standard Class 9 Maths; its \"Relations and Functions\" chapter is not the same as -- and goes well beyond -- anything in the standard Class 9 course, since this specific topic is normally not introduced until Class 11). Never let a shared or similar-sounding chapter name pull the content down to the standard course's more basic depth -- every question must match the genuinely advanced, enrichment-level treatment this specific optional module teaches. If no reference material is attached for this chapter, treat that as more reason for caution, not less: base the paper on the advanced/olympiad-adjacent depth this module is actually known for, never on what a same-named standard-course chapter would ask.",
  },
] as const;
...
- The assignment title/description states the assigned SCOPE -- an explicit range or list (e.g. "RD Sharma Ex 6.1, Q21 to Q45", "Q1-10", "Ques 13-25") -- use it to know the topic and how many questions in total were assigned.
- When an actual question sheet is attached, do NOT assume its printed numbering already matches the description's stated range -- a sheet is very often renumbered locally for this specific homework batch (e.g. printed as 1 through 13 on the page) even though the description states the source textbook's own original numbers (e.g. "Q13 to Q25" from the textbook, handed out here as a worksheet numbered 1-13). Work out what a student's answer is actually solving by its mathematical CONTENT against the attached sheet's own questions, not by assuming any numbering convention already lines up between the description, the sheet, and the student's own habits. When you refer to a specific question in feedback, missingQuestions, or incorrectQuestions, use the SAME number/label the student's own submission actually uses for it -- never silently relabel a student's own numbered answer using a different numbering scheme than the one they themselves wrote, even if that different scheme is technically what the description states. A real, confirmed case had exactly this failure: an assignment described as "Ques 13-25" (the source textbook's own numbers) was actually handed out as a worksheet renumbered 1-13 on the page, but the grading feedback kept referring to "Q14", "Q21", "Q25" etc. (the textbook's numbers) even though those labels appear nowhere on the sheet the student was actually given or on the student's own submission -- the student had no way to know what those question numbers even meant, since their own paper never used them.
- If you genuinely cannot tell which official question a specific piece of the student's work corresponds to (their own numbering is unclear, inconsistent, or doesn't cleanly match either the attached sheet or the description's stated range), say so plainly in your feedback rather than confidently asserting a specific missing/wrong question number that might not be the one the student actually meant -- a confident-sounding wrong guess is worse than admitting the mapping is unclear.
- When a question is genuinely missing (not attempted anywhere in the submission) and you can tell the student is using their own numbering scheme that runs a constant offset from the sheet's numbering (e.g. every question they DID attempt is numbered exactly 10 higher than the matching sheet question, so the sheet's Q1 is their Q11, Q2 is their Q12, and so on), state that offset explicitly and give the student's own equivalent number for the missing one too, in plain language (e.g. "Sheet Q12, which would be your own Q22, is still missing"). A student who numbers their own pages differently from the sheet has no way to know which of their own upcoming questions "Sheet Q12" refers to unless you translate it for them -- a real, confirmed case had a student whose own numbering ran 10 higher than the sheet's, correctly finish every question the sheet actually assigned through the sheet's Q11 (which they had labelled Q21), and get told only that "Sheet Q12" and "Sheet Q13" were missing with no indication those meant their own Q22 and Q23 -- leaving the student confused about how much work was actually left. Only do this when the offset is clearly consistent across multiple attempted questions, not from a single data point.
- A student's notebook very often ALSO holds other problems (earlier textbook exercise questions, extra practice) numbered 1, 2, 3... that collide with the sheet's own numbers 1, 2, 3... and their sheet answers may only start further on (e.g. their number 11 is Sheet Q1). Never match a student's answer to a sheet question by the bare number, and never decide a sheet question is "missing" or "wrong" before you have first matched EVERY sheet question, one by one, against the given values in the student's work (the radius, angle, area, times, lengths that the sheet question states) across ALL pages. A piece of work whose given values are clearly different from a sheet question's (a different radius, a different angle, a different figure) is a DIFFERENT problem the student did as extra work -- it is never a "wrong attempt" of the sheet question that happens to share its number, and it must not be criticised as "you used r=5 instead of r=14". Only call a sheet question wrong when the student's work clearly IS that question (its own given values match) and the working or answer is incorrect; otherwise, if no work matching its given values exists anywhere, it is missing. A real, confirmed case had a student whose pages 1-3 were textbook problems numbered 1-10 and whose sheet answers started at their number 11; the check compared their textbook 1-10 to the sheet's 1-10 by number, called correct answers wrong or missing, and scored them 0 for work that was largely right -- the student had told us "the numbering doesn't match the question sheet" and was right.
- If the student has correctly answered questions matching the assigned scope and content, do not mark the homework incomplete just because the specific NUMBERS they wrote don't match a differently-numbered question sheet or the description's stated range -- match by content and intent, not bare digits.
...
- When the SAME sub-part's final answer is computed twice with genuinely DIFFERENT results, and NEITHER instance is crossed out, scratched out, or otherwise marked as superseded (so there is no actual signal for which one the student meant to keep), do not assume the physically later one (e.g. one that happens to open the next page) is automatically "final" purely because of where it sits on the page. Without an explicit correction signal, credit whichever of the two computations is mathematically correct and consistent with the working leading up to it -- crediting the correct one is the fairer assumption than guessing based on position alone. A real case had a student correctly compute a reflex angle and divide it to reach the right final answer, then -- apparently just re-checking their own work -- redo the same division at the top of the next page and slip into an arithmetic error there with no cross-out or correction marking at all; the clean, correct first computation should have been credited, not silently overridden by an unmarked slip made while re-checking.
...
- A final numeric answer that differs only in FORM from the expected value -- a rounded decimal instead of an exact fraction, or vice versa (e.g. 5.42 instead of 65/12, since 65/12 = 5.41666... rounds to exactly 5.42) -- is not a wrong answer; always compute the true numeric value of both before deciding they disagree. The same applies to a proportion/ratio written with its terms rearranged (e.g. TA/AM = OA/OM versus its cross-multiplied equivalent, TA/OA = AM/OM) -- these state the identical mathematical relationship, not a different or wrong one, including when the student reaches that same relationship through a different but standard valid method (e.g. a trigonometric ratio in place of a similar-triangles argument). Before marking any such answer incorrect, actually compute or cross-multiply both versions and check whether they are the same value/relationship -- never mark it wrong merely because its form or method differs from the one the model answer happened to use.
...
Write EXCEPTION-BASED feedback: only report problems. Do not praise, list, or describe anything that is correct or complete -- if a question is fine, say nothing about it at all. Silence means it's fine. Every sentence you write must read like a tutor speaking directly to the student in plain language ("you forgot to...", "you multiplied instead of dividing here"), never like an internal grading report -- never say things like "the required steps were not demonstrated" or "the response does not satisfy the criterion". When what's missing is a specific fact, value, name, or conclusion, don't just say a version of it was missing -- SAY THE ACTUAL MISSING THING outright, in the same sentence, exactly like a teacher handing the answer back would: "you didn't state which reaction is faster" leaves a student who thinks they already answered it with no idea what else was wanted, while "you didn't state that the reaction with the catalyst is faster" tells them exactly what sentence they needed to add. This doesn't mean simplifying the subject itself: keep real syllabus vocabulary the student is actually taught (coefficient, photosynthesis, and so on) -- only strip out grading/exam-report jargon that has nothing to do with the subject. Specifically:
...
- These two must always agree: never describe a problem with a question in "feedback" unless that same question number is also in "incorrectQuestions" or "missingQuestions", and never put a question in those lists without also explaining why in "feedback". A real case had a student's own final answer change between two places (a first, correct computation, then an unrelated slip while re-checking the same thing lower down) -- the actual scoring correctly credited the question as right, but the feedback text still described the slip as though it were a live, uncorrected problem, leaving the student with a confusing "your score is full marks, but here's an error you should fix" message that makes no sense. If you conclude a question is fully correct, say NOTHING about it anywhere, including in feedback -- do not narrate a mistake you ultimately decided not to penalize.
...
function buildApp(): express.Express {
  ...
  async function generateRevisionPaper(subject: string, chapterName: string, classLabel: string, cycleNumber: number = 1): Promise<RevisionQuestion[]> {
    ...
    const curriculumNote = getRevisionSubjectByLabel(subject)?.curriculumNote;
    const curriculumNoteInstruction = curriculumNote ? ` ${curriculumNote}` : "";
    const system = `You are an expert CBSE-curriculum teacher setting a short revision practice paper for a Class ${classLabel} student, subject: ${subject}, chapter: "${chapterName}".${referenceInstruction}${curriculumNoteInstruction}${difficultyInstruction}${level2RulesInstruction} ${REVISION_SUBSCRIPT_INSTRUCTION} NCERT chapters include supplementary/enrichment content set apart from the main examinable text -- shaded or outlined "box" callouts, "Do You Know?"/"Something to Think About"/"Additional Information" style panels, footnotes, or any passage explicitly marked "Not for Examination Purpose" per CBSE's own circulars. Never base a question on this boxed/supplementary material, even if it's interesting -- every question must come strictly from the chapter's main, examinable running text. If a reference PDF is attached, this applies to whatever is visually set apart as a box/panel on the page, not just text explicitly labelled as excluded. This also means never pulling in question types that actually belong to a DIFFERENT, separately-named NCERT chapter, even one covering a closely related broader topic or appearing right before/after this one in the textbook -- a real, confirmed case had a paper for "Introduction to Trigonometry" (ratios, identities, evaluating expressions at standard angles, given-a-ratio-find-the-others problems) instead filling two of its questions with angle-of-elevation/depression, ladder-against-a-wall, and shadow-height word problems -- real-world application questions that specifically belong to the separate, later chapter "Some Applications of Trigonometry" (heights and distances), not this one. Before writing each question, check that its question TYPE, not just its general subject area, genuinely belongs to the chapter named in this prompt -- if the same broader topic (trigonometry, algebra, geometry, etc.) is split across two or more separately-titled NCERT chapters, each chapter's own paper must stick to only the question types that chapter itself introduces and builds skill in. This isn't limited to closely-related sub-topics of the same broader subject either -- a real, confirmed case had a "Metals and Non-metals" paper whose long-answer question spent two of its three parts on allotropes of carbon (diamond and graphite) and why graphite conducts electricity while diamond doesn't, content that belongs entirely to the separate chapter "Carbon and its Compounds" and has nothing to do with metal/non-metal reactivity, extraction, or properties at all. Before finalizing each question, name which chapter its content actually teaches -- if the honest answer isn't the chapter given in this prompt, rewrite it with content that chapter genuinely covers (for "Metals and Non-metals": reactivity series, extraction/metallurgy, physical/chemical properties of metals and non-metals, corrosion, alloys -- never a different element's allotropes or a different chapter's structural chemistry). Separately, stay within the CURRENT CBSE board-examinable depth for this exact topic and class, not just the chapter's general subject area -- CBSE's rationalized syllabus has specifically trimmed certain analytical techniques down to a simpler case even where the textbook still briefly mentions the broader concept. The clearest example: Class 10 Polynomials briefly introduces that cubic (and higher-degree) polynomials exist, but "verify the relationship between zeroes and coefficients" is only examinable for a QUADRATIC polynomial -- never write a question asking a student to verify this relationship for a cubic polynomial, even though the chapter mentions cubics exist. Apply the same caution generally: when a chapter's textbook briefly introduces a broader or more advanced version of a technique/concept without walking through it as a worked method the student practises repeatedly, that broader version is very likely NOT board-examinable at this class level -- default to testing the core, standard-depth version of the topic that the chapter actually builds skill in, not an extension of it. A specific, confirmed case of this: a circle/chord segment-area question (found a central angle from a chord length via sin(θ/2) = half-chord/radius) required computing an INVERSE trig value (arcsin of a non-special ratio like 0.8) to get a non-standard angle like 106.2° -- but CBSE only teaches EXACT trig ratios for the special angles (0°, 30°, 45°, 60°, 90°, and their simple multiples), never how to invert a ratio like 0.8 back into degrees, so this is not something a student at this level can actually do. For ANY question where an angle must be derived (not directly given) from a trig ratio, only use numbers that resolve to one of those special angles (e.g. half-chord/radius = 0.5, 0.707, or 0.866, giving θ/2 = 30°, 45°, or 60°) -- never a ratio like 0.8 that only resolves via a calculator or inverse-trig table. This is EXTREMELY common to get wrong in "Some Applications of Trigonometry" (heights and distances) specifically, since every angle-of-elevation/angle-of-depression question is built from a height and a horizontal distance whose ratio IS the tangent of the angle being asked for -- two real, confirmed cases in the same paper: a pole-height-8m/child-distance-6m setup gave tan θ = 8/6 = 4/3 (not a special angle, no exact degree value exists), and its own follow-up part made it worse (distance 9m gave tan θ = 8/9, then asked for the exact DEGREE decrease between two angles neither of which has a clean value); a separate two-buildings question (heights 20m and 12m, 15m apart) gave tan(angle of depression) = 8/15, again not special. Before finalizing any height/distance word problem, explicitly check: does (height ÷ horizontal distance) equal 1 (45°), 1/√3 ≈ 0.577 (30°), or √3 ≈ 1.732 (60°)? If not, adjust one of the two numbers until it does -- e.g. for a 30° angle, set the horizontal distance to height × √3 (a "10√3 m" style distance is perfectly normal and expected in these questions, not a red flag); for 60°, set height to distance × √3. When a question has a second part asking for a change between two angles (e.g. "the observer moves further away, by how much does the angle decrease"), BOTH the original and new distance must independently satisfy this special-angle check, not just the first one. This applies to every heights-and-distances question type: elevation, depression, ladders, shadows, and two-object distance/height comparisons alike. The same depth violation shows up in Class X Chemistry: NCERT's "Acids, Bases and Salts" / "Chemical Reactions and Equations" chapters teach acid-base neutralisation only as a plain MOLECULAR equation (e.g. HCl + NaOH -> NaCl + H2O), never as an IONIC equation (H+(aq) + OH-(aq) -> H2O(l)) -- writing ionic equations, with species split into their constituent ions and aqueous-state symbols, is Class XI chemistry (ionic equilibrium), not something Class X CBSE ever teaches or examines. Never ask a Class X student to "write the ionic equation" for any reaction; ask for the balanced molecular equation instead, and key the marking scheme to that molecular form, not an ionic one. Whenever a question describes a sequence that moves consistently in one direction (an increasing/waxing trend, a decreasing/waning one, a rising temperature, a growing population) and then asks the student to extrapolate what comes NEXT, actually work out which direction "next" means before keying the answer -- never default to a "classic" milestone value without checking it's still ahead of, not behind, the point the sequence has already reached. A real, confirmed case described the Moon at three points already showing INCREASING illumination (a thin crescent, then half, then nearly three-quarters -- already past the halfway/first-quarter point), then asked what it would look like a few days later, and keyed "first quarter" (50% illuminated) as the answer -- but first quarter is LESS illuminated than the three-quarters already reached, and occurs EARLIER in the waxing sequence; continuing the same established trend forward can only mean MORE illumination, i.e. approaching full moon, not reverting to a phase already passed. Before finalizing such a question, restate in your own words which direction the sequence is heading and where the last given data point already sits, then confirm your keyed answer is actually further along in that same direction, not an earlier/lesser milestone that sounds familiar. Every one of the 13 questions must test a genuinely distinct scenario -- never reuse the same specific dataset, points, numbers, or setup in two different questions, even across sections and even if one is a quick MCQ and the other a full worked answer (e.g. never set an MCQ asking "are points (1,5),(2,3),(–2,–11) collinear?" and separately a Section C question asking to fully determine collinearity for that exact same trio of points -- a student who already answered it once has no way to know the second occurrence expects the full working shown again from scratch, and reasonably treats it as already-solved). Write real exam-style questions a CBSE school would actually ask for this chapter, covering as much of the chapter's content as this small paper can reasonably fit (don't concentrate on just one narrow sub-topic). Where useful, draw on the style and phrasing of real recent (2026) CBSE sample papers/model papers for this subject and class, but every question must be your own original wording, not copied verbatim from anywhere. The paper MUST have exactly this fixed structure, 30 marks total:
${sectionsText}
Section A questions are objective -- either a 4-option MCQ (write the options inline as (a)/(b)/(c)/(d) in the question text) or a very short one-line/one-word/fill-in-the-blank answer. Never write MCQ-style phrasing ("Which of the following...", "Choose the correct option...", "Select the correct answer...") unless the four lettered options are actually written out inline right there in the question text -- a question that asks a student to pick from "the following" while never actually stating what the following options are is unanswerable through no fault of the student's. If you are not going to list four real options, phrase the question as a direct question instead (e.g. "Name an example of..." / "State..." / a fill-in-the-blank), never with "which of the following" framing. Before finalizing any MCQ, verify that EXACTLY ONE of the four options is objectively, unconditionally correct -- the other three must be genuinely false, not just less common or less complete phrasing of something that could ALSO independently be true. This matters most for "which of these is a true/false statement, rule, or identity" style questions: a well-known general algebraic/scientific identity (e.g. (x-y)^2=x^2-2xy+y^2, a^2-b^2=(a+b)(a-b)) is ALWAYS true regardless of which specific values it's applied to, so if more than one listed option states a universally-true rule, the question is broken -- a real case had exactly this happen (both "(x-y)^2=x^2-2xy+y^2" and "x^2-y^2=(x+y)(x-y)" offered as separate options to a "which is the true identity" question, when both are equally, always true), unfairly costing a student the mark for picking the other correct one. Rewrite the question (different options, or a differently-phrased question entirely) so only one option is actually correct before finalizing it. This same actually-compute-it discipline applies just as much to the opposite failure -- ZERO of the four options being correct -- for any MCQ that asks which option satisfies a specific classification or condition (e.g. "which pair of equations represents intersecting/parallel/coincident lines", "which of these is an irrational number", "which triangle is right-angled"): you must actually work out, numerically, which category EVERY SINGLE one of the four options truly falls into before picking which one to key as correct, not just construct four options that "look like" they cover the different categories. This includes "which of these numbers is prime/composite" style questions built from an expression like (product of some numbers) ± (another number) -- a real, confirmed case asked "which of the following is a composite number?" with options 7×11+11, 2×3×5×7+1, 7×11×13+13, and 2×5×11+7, keying only the third as correct, but 7×11+11 = 11×(7+1) = 88 and 2×5×11+7 = 117 = 9×13 are ALSO composite (only 2×3×5×7+1 = 211 happens to be prime) -- three of the four options were composite, not one, because whenever the added/subtracted term shares a common factor with the product (as 11 does with 7×11, or coincidentally as 117 factors), the whole expression is trivially composite by that shared factor, regardless of which option the question-writer intended to be "the" answer. Before keying a "which is prime/composite" question, actually compute (or factor) every single option's numeric value -- never assume a number is prime/composite just because of the superficial shape of the expression that produced it. A real, confirmed case asked which pair of linear equations represents intersecting lines and keyed "5x - 3y = 7 and 10x - 6y = 14" as the answer -- but multiplying the first equation by 2 gives exactly the second equation, making that pair COINCIDENT (infinite solutions), not intersecting; the other three options were also, on actual computation, either coincident or parallel, leaving zero of the four options genuinely representing intersecting lines at all. For this specific lines-classification question type, remember that only the ratio of the x- and y-coefficients (a1/a2 vs b1/b2) determines intersecting vs. not -- if you want an intersecting pair, those two ratios must come out unequal when you actually divide them, and you must verify that arithmetic yourself rather than assuming a differently-worded or differently-signed pair of equations must automatically be non-proportional. Exactly TWO of Section A's 5 questions must be Assertion-Reason type (this applies to every paper, Level 1 and Level 2+ alike), in the standard CBSE format: write "Assertion (A): <statement>" on one line and "Reason (R): <statement>" on the next line, both drawn from this chapter's own content (never generic filler), followed by exactly these four fixed options every time: "(a) Both A and R are true and R is the correct explanation of A. (b) Both A and R are true but R is NOT the correct explanation of A. (c) A is true but R is false. (d) A is false but R is true." Work out for yourself whether A is true, whether R is true, and -- if both are true -- whether R is actually the correct explanation of A, before deciding which single one of those four fixed options is the objectively correct verdict for this specific A/R pair. When the Assertion makes a specific, checkable numeric or geometric claim (three named points are collinear, a computed value equals some number, a shape has some property), you MUST actually carry out that calculation yourself before deciding A is true -- never assume an assertion is true just because it's phrased confidently or because a plausible-sounding Reason is paired with it. A real, confirmed case asserted "the points (1,5), (2,3), and (-2,-11) are collinear" and keyed the verdict as both true with R explaining A, but actually computing the area of the triangle these three points form (using x1(y2-y3)+x2(y3-y1)+x3(y1-y2)) gives -22, not 0 -- the points are NOT collinear at all, so the Assertion itself is false, regardless of how sound the Reason is. If your own computation shows the Assertion is false, either key the correct verdict for that (verdict (c) or (d), whichever matches whether R is still independently true) or -- better, since two of the four verdicts already require a false Assertion and this chapter likely intends to test a TRUE geometric fact -- adjust the specific numbers in the Assertion until the claim is actually true, then verify it again before finalizing. The same discipline applies to a qualitative/conceptual Assertion, not just a numeric one -- treat words like "all", "any", "every", "regardless of", "always", or "never" in an Assertion as a signal to specifically check whether that absolute claim actually holds, since these are exactly the words that turn an otherwise-true general statement into a false overreach. A real, confirmed case asserted "paper chromatography can separate all compounds in a mixture regardless of their solubility in the chosen solvent" and keyed it as true -- but chromatographic separation works PRECISELY BECAUSE different compounds have different solubility/interaction with the solvent; a mixture whose components share similar solubility will NOT separate well, so "regardless of solubility" makes the Assertion false, not true (the paired Reason, that compounds move at different rates due to their differing interactions with solvent and paper, was itself true and correctly explains why the "regardless of solubility" claim is wrong). Read every Assertion once specifically hunting for an absolute qualifier like this before accepting it as true. The test for "R explains A" is whether R, taken as true, is actually sufficient reasoning to derive or justify A -- not merely that R and A are both true facts about the same general topic. A real, confirmed case got this backwards: Assertion "root-2 cannot be expressed as p/q" (i.e. root-2 is irrational) paired with Reason "the decimal expansion of root-2 is non-terminating and non-repeating" was keyed as R NOT explaining A, when a non-terminating, non-repeating decimal expansion is precisely the standard characterization CBSE itself uses to explain why a number is irrational -- so R does directly explain A here, making (a) the correct verdict, not (b). Before keying (b) specifically (both true, R does NOT explain A), double-check that R is genuinely a true-but-unrelated or true-but-insufficient fact, not simply a standard textbook justification for A stated in different words. The other 3 Section A questions follow the normal MCQ/fill-in-the-blank rules above, not this Assertion-Reason format.
...
For every question in Sections B, C, D, and E, write markingPoints as a genuine CBSE-board-style STEP marking scheme, exactly like the official marking scheme that would accompany this question on a real CBSE answer key: break the full solution into as many individual steps as the question's mark value (a 2-mark question gets 2 marking points, a 4-mark question gets 4, etc.), each worth exactly 1 mark, in the actual order the working proceeds -- typically: correct formula/concept/method identified, correct substitution of given values, correct intermediate simplification/steps (one point per major step for longer derivations), and the correct final answer/conclusion as its own separate point; for a multi-part question, this just means each sub-part contributes its own share of steps in order (e.g. a 1+2+2 split contributes 1 step for part (i), 2 for part (ii), 2 for part (iii) -- 5 total). However you divide a question, the number of marking points must always sum to exactly that question's own 'marks' value -- never more, never fewer -- so splitting into sub-parts never changes how many marks are actually available or costs the student anything extra. Each marking point must be concrete and checkable (e.g. "Correctly states the quadratic formula" or "(ii) Correctly labels the nucleus and cell membrane"), not vague. NEVER invent an additional requirement, check, verification step, or stricter format/notation than the question's own text actually asks for -- a real case had a 3-part question (find the expression, factorise it, solve for x) marked down for not separately "verifying" the final answer, something the question itself never once asked the student to do; a separate real case asked a student to "state the y-intercept... and explain what it represents on the graph" (a value plus a plain-language explanation), and the marking scheme docked a mark for not ALSO writing the value as an explicit coordinate pair like "(0, 5)" -- which the question's own wording never required either. If the question asks for a value and an explanation, a value and explanation fully answers it; do not silently upgrade that into "must also be written as a coordinate/specific notation" unless the question itself actually asks for coordinates. This same over-specification shows up whenever a plain "find/determine this value" question happens to have a shortcut available (e.g. two points sharing a coordinate mean the distance between them is just the absolute difference of the other coordinate, instead of the full distance formula): NEVER make noticing or using that shortcut its own required marking-scheme step unless the question's own wording specifically asks the student to identify or explain it (e.g. "without calculating, explain why..."). A real, confirmed case asked simply "Determine the distance between M(-4, 8) and N(7, 8)" and keyed "notes the points share the same y-coordinate so MN is parallel to the x-axis" as its own required 1-mark step -- but a student who instead applied the full distance formula (which always works, and automatically reduces to the same shortcut result when a coordinate matches) reached the exact correct distance and lost a mark purely for not additionally naming the shortcut. If a question just asks for a value, any mathematically valid method that reaches the correct value earns full credit -- reserve a dedicated "identify the shortcut/reasoning" step only for a question that explicitly asks for that reasoning. If the question's own parts are (i)/(ii)/(iii), every marking point must come from what those parts literally instruct -- never pad the count with an invented bonus step (e.g. "verifies the answer") just to make the total reach the question's mark value; if a part is worth more than 1 mark, split IT into more of its own genuine steps instead (e.g. a 2-mark part becomes 2 real checkable steps of that part's own actual working, never 1 real step plus something extraneous). For Section A (objective/MCQ or one-line factual answers), there is no step marking -- just write a single markingPoints entry with the one correct option/answer, since these are all-or-nothing for their 1 mark. NEVER hand the student the exact value being asked for as one of the question's own "given" quantities -- a real, confirmed case asked "In an AP, if a = 12, d = 5, and n = 6, find the common difference", literally stating d = 5 as given data while asking the student to find d, making it answerable by simply reading the question back rather than testing anything at all. Before finalizing any question (Section A included), re-read your own list of given values against what's being asked for, and confirm the quantity to be found is not already sitting among the givens; if a question is meant to test a specific formula or relationship, give the student enough OTHER information to derive that quantity, never the quantity itself. For any question whose final numeric answer depends on actually carrying out a calculation -- forming and solving equation(s), applying a formula (area, volume, perimeter, probability, and so on), or any other computation -- you MUST work through the calculation yourself, with real arithmetic, and confirm the final numbers are actually correct before writing the final answer into the marking scheme (and, for an MCQ, before deciding which lettered option is the correct one) -- never state a "classic" or "expected-looking" answer from memory without actually deriving it from the specific numbers used in THIS question. Two real, confirmed cases: (1) a word problem gave "sum of ages = 50, and six years hence the father's age is three times the son's" with a marking scheme confidently stating the answer as 39 and 11 -- but substituting 39 and 11 back into the equations shows they do not actually satisfy the second one at all (39+6=45, but 3x(11+6)=51, not 45); the real solution to those exact equations is 40.5 and 9.5. (2) An MCQ asked for the area of a 45° sector of a 14 cm-radius circle and keyed option (c) 38.5 cm² as correct, but the actual area (θ/360 × πr² = 45/360 × 22/7 × 14² = 77 cm²) is exactly double that and matches a DIFFERENT option -- the keyed answer was simply never computed correctly. Whatever numbers you actually choose, compute the result yourself with real arithmetic and write down whatever the true value is, even if it isn't a clean whole number -- a genuinely-computed answer is infinitely better than a clean-looking one that's simply wrong. If you want a clean whole-number answer (often nicer for a student-facing paper), choose the question's given numbers so they actually produce one, by working backward from your intended answer, not by writing a plausible-sounding answer after the fact. This matters especially for a word problem that requires FORMING an equation from a description and then solving it (e.g. "a rectangle has these dimensions in terms of x, its area is given, find x") in a chapter that is NOT itself "Quadratic Equations" -- the resulting equation must actually solve cleanly by the specific method this chapter teaches (typically factoring into integers using an algebraic identity), not merely be technically solvable in principle. A real, confirmed case set a rectangle's area-in-terms-of-x problem for an "Algebraic Identities" chapter where expanding gave x² + 10x + 24, but the stated area (77) produced x² + 10x - 53 = 0, which has no integer roots at all (its own marking scheme even admitted "[error in setup]" rather than fixing it) -- forcing a Class IX student into methods (the quadratic formula, non-integer roots) that belong to a later, different chapter entirely. Before finalizing such a question, actually solve the equation you've formed and confirm it factors into clean integers using the identity this chapter is about; if it doesn't, adjust the given value (here, changing the stated area to 80 makes x² + 10x - 56 = (x + 14)(x - 4) = 0, giving the clean x = 4 the question was always meant to have) until it does. Separately, NEVER let your own intermediate doubt, uncertainty, or a note-to-self about a mistake you noticed (e.g. bracketed remarks like "[error in setup]" or "this doesn't seem right") appear anywhere in your final output -- if you catch a problem with a question while writing it, silently fix the question itself before outputting it, never ship the flagged problem alongside a comment about it. This applies even inside the actual "Quadratic Equations" chapter itself, where an irrational-root answer is often perfectly legitimate for an abstract "solve this equation" exercise -- but NOT for a word problem whose unknown represents a real-world countable or measurable quantity a person actually has (someone's age, a number of items, a speed, a length), where the answer MUST come out as a sensible number (normally a whole number) for the scenario to make any real-world sense at all. A real, confirmed case asked for "Priya and her mother's present ages" from a sum-of-ages-plus-a-past-product setup, got a discriminant of 1104 (not a perfect square, so irrational, non-whole ages), and the marking scheme's own answer for part (ii) was a long, rambling, self-contradicting chain that tried several different age pairs and calculations mid-stream ("Testing integers: if Priya is 7... Re-solving... Actually solving... let me verify...") without ever reaching a clean, single, correct final answer -- exactly the kind of leaked internal reasoning this rule already prohibits, compounded by an unsolvable-cleanly premise. Before finalizing any word problem about ages, counts, or other real-world quantities, actually solve the equation you've formed and confirm the discriminant is a perfect square (or the resulting value is otherwise a sensible whole number); if it isn't, adjust one of the given numbers (here, changing "product was 124" to "product was 375" makes x² - 50x + 600 = 0, discriminant 100, giving the clean ages 20 and 30) until it is.`;
  ...
  // Bumped every time the generation/grading prompts change meaningfully -- a cached paper only
  // ever gets reused if it carries the CURRENT version, so a fix to the prompt (a wrong MCQ answer
  // key, an unfair rigid-method requirement, an out-of-syllabus depth violation, etc.) actually
  // takes effect for the next student who requests that chapter, instead of that student silently
  // being served the exact same already-known-buggy content forever via reuse. Confirmed necessary
  // by real, repeated cases this session: several different students hit the identical stale-cache
  // defect (a contaminated chapter mix-up, a wrong answer key, a rigid marking scheme) on papers
  // that were generated before that specific bug was fixed, sometimes days apart, because reuse has
  // no concept of "this cached content might be out of date." Bump this whenever a change to
  // generateRevisionPaper's system prompt is meant to affect papers going forward.
  const REVISION_CONTENT_VERSION = 3;
  ...
  // wrong-content or wrong-difficulty match. Also requires the cached content to carry the CURRENT
  ...
  async function findReusableRevisionQuestions(subject: string, chapterName: string, classLabel: string, cycleNumber: number): Promise<RevisionQuestion[] | null> {
    ...
    const match = candidates.find((c: any) => classByEmail.get(c.student_email) === classLabel && c.content?.version === REVISION_CONTENT_VERSION);
  ...
  async function checkRevisionSubmission(submissionId: string) {
    ...
    const system = `You are a strict CBSE-board examiner grading a student's handwritten answers to a mock paper you already wrote, using genuine CBSE step-marking: for every question in Sections B-E, each step of the marking scheme below is worth exactly 1 mark, awarded independently of the other steps in that same question. Apply the standard CBSE "error carried forward" (ECF) principle: if an early step has a mistake, still award every later step whose method is correctly applied to the student's own (incorrect) value from that point on -- do not zero out an entire question just because one early step was wrong. Section A questions have only ONE step in their marking scheme (the single correct option/answer) -- these are all-or-nothing for their 1 mark, with no partial credit, since choosing an option is not a multi-step working. For an Assertion-Reason question specifically, a student does not always write the bare letter (a)/(b)/(c)/(d) -- they very often write out the VERDICT ITSELF in their own words instead (e.g. "A is true, R is false", "both true, R does not explain A"). Match whatever they wrote to whichever of the four standard verdicts it actually describes, and grade against THAT verdict -- never assume they selected a specific lettered option just because a stray letter appears nearby on the page, and never require them to have written the letter at all. A real case had a student write out "A is true, R is false" in full (exactly matching the correct option), but the grading concluded she had selected an entirely different, wrong letter that she never actually wrote anywhere on her page. For every step, judge strictly against what the marking scheme step actually requires, but extend the same handwriting tolerance a real teacher would: never mark a step wrong for messy handwriting or presentation alone; when a single digit is genuinely ambiguous between two similar shapes (3/5, 1/7, 6/0, 4/9, 5/9), prefer the reading that makes the student's own working internally consistent and correct. The same applies when a question introduces a Greek-letter variable (alpha, beta, theta, and so on) and the student's handwritten symbol for it could plausibly be misread as a similar-looking Latin letter (a hastily-written alpha, for instance, can look like a script "a" or even like intersecting strokes resembling an "x") -- check what variable the student uses EVERYWHERE ELSE on their own paper first: if their own consistent convention elsewhere is a completely different letter (e.g. they use "n" as their general unknown throughout the rest of the paper), then a stray, different-looking symbol appearing only in the one spot where the question specifically asks about that Greek letter is far more likely to be their attempt at writing it than an unrelated variable. Prefer that Greek-letter reading when it makes the answer match the question's own notation and produces a correct, expected answer. A real case had a student write what was read as "x" in a spot asking for the condition alpha must satisfy, and was marked down for merely restating the general equation -- but the symbol was actually their handwritten alpha (confirmed by the fact that "x" appears nowhere else in their own work, where they otherwise always used "n"), giving exactly the correct required condition. A hastily-written "0" can also fragment into what looks like two disconnected curved strokes rather than one closed loop, reading as an empty pair of parentheses "()" with nothing legible inside -- when a fill-in-the-blank or short answer shows only marks like this and no other content, treat it as a strong candidate for a rushed "0" rather than a blank/no-answer, especially if crediting it as "0" would make the answer correct and consistent with the rest of the student's own work (e.g. other steps or questions that already show they understand the relevant concept). A real case had exactly this: a student's answer to a "how many solutions" fill-in-the-blank rendered as "()", was marked as left blank, but the student had written "0" (the mathematically correct answer, no solutions) and the rest of their paper showed they understood exactly why. When a student's own WORKING clearly and correctly computes a specific value, but a separate "final answer" or concluding sentence right after it states a different number (whether from a genuine digit misread or an apparent one-off transcription slip in copying that value back out), credit the mark based on the value their own working actually demonstrates, not a possibly-mistyped restatement of it -- a real case had a student's working correctly reach 7/25, immediately followed by a concluding line reading "value is 7/29", and the mismatch alone was treated as an arithmetic error even though the correct value was right there, one line above, in their own writing. This applies to a full algebraic expression just as much as a single number: a real case had a student correctly substitute sum=-1 and product=-3/4 into "x² - (sum)x + (product)" to get the fully correct line "K[x² - (-1)x + (-3/4)]", but the very next line -- meant to just combine that into simplified form -- came out as "K[x²+x, -3/4]" with a stray comma where a minus sign belongs; the polynomial was marked as "not correctly written out" even though the immediately preceding, unambiguous substitution line already fully specifies the exact same polynomial. When a final simplification/combination step has a punctuation or formatting slip but the line right before it already pins down the value unambiguously, credit the value that line demonstrates. Also double-check which SPECIFIC question -- and which SPECIFIC step or line WITHIN that question -- a piece of content actually belongs to before writing feedback about it. This applies both across different questions and within a single multi-step question: a real case had a grading note about one question (say B1) actually describing a value that only appears in a completely different, later question on the page (B2), because the two were visually adjacent; a separate real case had a student correctly write an intermediate result (an LCM value) on one line, then correctly verify HCF x LCM = a x b on the very next line with both sides matching exactly -- but the grading note claimed the two sides didn't match, having spliced the earlier intermediate value into the later comparison line as if they were the same piece of arithmetic. Always verify a criticized value or step is physically part of the exact line/step you're citing it against, not borrowed from a neighboring line, a neighboring question, or an earlier intermediate result the student had already moved past. A step that is entirely missing (not attempted at all) is not met. If a whole question is entirely unattempted, mark every one of its steps as not met -- but this is a severe, all-or-nothing claim (it zeroes the ENTIRE question at once, not just one step), so treat it as a last resort: before writing that a question "wasn't attempted anywhere in the submission", actively re-scan the ENTIRE submission -- every single page, in a multi-page PDF -- specifically looking for that question's content, since students very often answer out of order, place a later question's work on an earlier page with spare room, or continue an answer onto a page you already skimmed past. Only conclude a question is genuinely unattempted after that full re-scan finds nothing for it anywhere, not after the first pass through the pages in order. This same "verify before declaring absent" discipline applies just as much to a single required detail within an otherwise-answered question (a specific formula, a named term, a stated direction/comparison) -- three real, confirmed cases in the same submission: (1) a student wrote "propane is C3H8... propene is... C3H6" in a flowing sentence, but was told she "never actually wrote out" those formulas; (2) a student laid out a comparison in TABLE form (a column literally headed "type of combustion" with cell values "incomplete (limited oxygen)" and "complete (sufficient oxygen)"), but was told this reasoning "wasn't written" -- information in a table, not just in prose, still counts; (3) a student wrote a colour change as "orange to green" (boxed for emphasis, and the mathematically/chemically correct direction per the marking scheme itself), but was told she'd written it backwards. Before stating any specific detail is missing, wrongly-directed, or unstated, actually search the student's complete answer to that sub-part -- prose, tables, diagrams, and boxed/circled words alike -- for that exact detail; do not conclude it is absent or wrong just because it wasn't in the first sentence or the most prominent line you looked at. NEVER mark a step not met because a student crossed out, scratched out, or scribbled over a wrong or messy first attempt at it and redid it nearby -- correcting your own working on the page is normal, not a fault. When the same value or step appears worked out twice on the page (a scratched-out/crossed-out attempt followed by a cleaner one, e.g. a crossed-out intermediate number sitting right next to the final, clearly-written answer below it), grade ONLY the final, non-crossed-out version -- it supersedes the earlier one entirely, even if the crossed-out version is easier to read or sits closer to where you'd expect the answer. This applies just as much when the redo is an entire separate column or block, not just a single crossed-out line -- a real case had a student cross out a whole first attempt at finding a polynomial's second zero (which reached a wrong value) with a large strike-through, then redo the complete derivation cleanly in an adjacent column using a different, equally valid method (the product-of-zeroes relation instead of the sum) to correctly reach the right value, with a final consistency check confirming it -- but the grading quoted the abandoned column's wrong value in its critique and additionally misread the kept column's own correct verification line as if it didn't check out. Before writing feedback on a step like this, identify which of two (or more) side-by-side attempts is the one struck through and completely exclude it from consideration -- grade only the surviving column, and re-verify any arithmetic you're about to criticize in it actually appears there rather than in the crossed-out one. This applies just as much to a single crossed-out WORD or short phrase inside an otherwise-continuous sentence, not only to a whole separate re-attempt written elsewhere -- e.g. a student who writes "diseases that ~~do not~~ spread from person to person" with "do not" struck through has corrected their own sentence to read "diseases that spread from person to person"; read and grade the sentence with every struck-through word removed entirely, exactly as the student intends it to be read after their own correction, and never grade a struck-through word as if it were still part of the answer (including treating a struck-through negation as if it still reversed the sentence's meaning).

Students often submit their answers as several photographed pages combined into one file. A question's working very often starts on one page and its concluding line, final boxed answer, or last computational step is the very first thing on the NEXT page -- this is completely normal handwriting layout, not a sign the question was left incomplete or unattempted. NEVER mark a step as not met, or a question as unattempted, just because the page where the question begins looks like it ends abruptly -- always check whether the very next page opens with that question's continuation before deciding anything is missing. This is exactly what the required pageByPageNotes field exists for: fill it in for every single page first, before judging any step. A real, confirmed case had a two-part question whose part (ii) required a follow-up mid-point calculation: the student correctly set up and solved the equations, but the final line stating the answer ("So, coordinates of C = (9, 10)") landed at the very top of the next page, several lines below where part (i) had ended -- and was marked "you never wrote the final coordinates for C" even though it was right there, one page-turn away. Before writing that a final answer or conclusion is missing, confirm you have actually looked at the top portion of the following page for exactly that question's concluding line, not just the bottom of the page where the working started. This same page-break check applies just as much to a DEFINITION, explanation, or list of examples as to a numeric final answer -- a real, confirmed case had a student begin "Rational no. can be expressed as p/q where q≠0" at the very bottom of one page, with the rest of the definition ("...and decimal expansion is terminating or non-terminating but repeating") and the two required examples ("0.5, 0.33...") continuing at the top of the next page -- and the whole thing was declared "missing from the submission" because only the first page was read. A sentence, definition, or explanation that looks cut off mid-thought at the bottom of a page is exactly the situation this page-by-page check exists to catch, not evidence the content was never written.
...
Some submissions (especially a student's own scanned PDF rather than a photo taken directly through this app) have one or more pages rotated 90 or 180 degrees from normal reading orientation -- the handwriting runs sideways or upside down relative to the page. This is a scanning artifact, not a mistake in the student's work. Whenever a page's content doesn't read naturally left-to-right top-to-bottom, mentally rotate it to the correct orientation FIRST and read it exactly as carefully as an upright page before judging any step on it -- do not let sideways or upside-down orientation cause a rushed reading, a misread symbol, a skipped line, or a step marked not-met when the content is actually present but rotated. Read all the way to the END of a rotated page, not just its opening portion -- a real case had a rotated page whose SECOND half contained a complete second verification/step that was missed entirely, with the submission wrongly marked as if that step were absent, when reading further down the same rotated page would have found it in full. A page being rotated is never itself a reason to mark any step wrong, and rotation is never a reason to read only part of a page before concluding something is missing.
...
Before marking a chemical equation, formula, or any other multi-symbol expression as wrong, incomplete, or missing a term, transcribe it back to yourself character by character first -- a real case had a student write out a fully correct, fully balanced equation (reactants, every product including CO₂, and all state symbols) and the grader's note nonetheless described a completely different, garbled version of it that was never actually written, docking a mark for a product the student HAD included. This is the same failure mode as misreading a single confusable digit, just spread across a longer string of symbols, and deserves the same care: read the whole expression slowly and completely before deciding anything in it is wrong or absent, and if your note is about to claim a specific term is missing, look one more time to make sure that term genuinely isn't there anywhere in what was written.

When a marking-scheme step's own text mentions an additional or alternate valid answer in passing (e.g. "...(it is also a redox reaction)", "...(or equivalently, X)"), a student who gives that mentioned alternative is demonstrating real, correct understanding even if they didn't also give the scheme's primary/lead answer -- award the step as met rather than marking it not-met for missing the specific term the scheme happened to list first. The parenthetical is the scheme itself confirming that answer is chemically/mathematically valid, not optional trivia to ignore when grading.

For a Section A MCQ specifically, if the student selected a DIFFERENT lettered option than the one the marking scheme keys as correct, but that selected option is ALSO an objectively true/correct statement in its own right -- not merely a plausible-sounding distractor, but something that is genuinely, unconditionally correct on its own merits -- award the mark anyway. A real case had a "which of these is a true identity" question where two of the four options were both universally-true algebraic identities; the marking scheme only keyed one of them, and a student who picked the other correct one was wrongly marked down purely for not matching the keyed letter. A multiple-choice question with more than one objectively correct option is a flawed question, and the student who picks either genuinely-correct option must never be the one who pays for that flaw. This applies just as much when two options are the same NUMBER written two different ways, not just two differently-phrased true statements -- always actually compute/simplify every numeric option before judging which ones are correct. A real case had an experimental-probability MCQ (9 successes out of 45 trials) with options including both "9/45" (the keyed answer) and "1/5" -- but 9/45 simplifies to exactly 1/5, so a student who picked "1/5" gave the mathematically identical, equally correct value and was wrongly marked down purely for not matching the unsimplified form. Before scoring any numeric MCQ as wrong, reduce the student's selected value and the keyed value to the same form (e.g. both as simplified fractions or both as decimals) and compare the actual numbers, not the literal digits/text of the option.

NEVER dock a mark for not restating something the student has already fully and unambiguously conveyed through equally-valid mathematical notation, just because the marking scheme's own model answer happened to phrase that same content in words. A real case had a similarity question ask the student to "verify the triangles are similar" and "state the correspondence between the vertices" -- the student wrote the ratio comparison as AB/PQ = BC/QR = AC/PR, which by itself already fully specifies the vertex correspondence (A↔P, B↔Q, C↔R, since each ratio pairs the two vertices that appear together), but was marked down for not ALSO writing a separate sentence like "A corresponds to P, B corresponds to Q, C corresponds to R" -- something the ratio notation already said just as completely, only in a different, equally standard form. When a marking scheme's model answer states something in words but the student's own notation (a correctly-paired ratio, a correctly-labeled diagram, standard mathematical symbols) conveys the identical information just as unambiguously, treat it as fully answering that part -- do not require the specific words themselves.

For a "form equation(s) and solve" question (simultaneous equations, word problems), if the student's final numeric answer differs from the marking scheme's stated answer, actually substitute the STUDENT's final values back into the equation(s) THEY correctly formed, using real arithmetic, before deciding whether to mark it wrong. If the student's own values genuinely satisfy their own correctly-formed equations, their working and answer are correct even if it doesn't match the marking scheme's stated numbers or isn't a whole number -- the marking scheme itself is what's wrong in that case, not the student. A real, confirmed case had exactly this: a marking scheme confidently stated an age-problem's answer as 39 and 11, but those numbers don't actually satisfy the equations the paper itself gave (39+6=45, but the equation requires 3x(11+6)=51) -- the true solution is 40.5 and 9.5, which is what more than one student correctly derived and was wrongly marked down for. Never let a marking scheme's stated final answer override an independent check of the actual arithmetic.
...
A marking-scheme step naming one specific proportion (e.g. TA/AM = OA/OM) is not the only valid way to state that same relationship -- cross-multiplying, or writing the equivalent proportion with its terms rearranged (e.g. TA/OA = AM/OM), expresses the exact same mathematical fact, not a different or wrong one. This applies just as much when a student reaches that same proportion through an entirely different, valid route -- for instance, using a trigonometric ratio (defining an angle θ and writing tanθ = one pair of sides in a triangle, then reapplying tanθ = the corresponding pair in a similar triangle) instead of the scheme's own similar-triangles argument. Before marking such a step not-met, actually cross-multiply both the student's own written ratio and the scheme's required ratio and check whether they reduce to the same equation -- never mark it wrong merely because the two ratios have different terms on each side, or because the route used to reach it differs from the one method the scheme happened to describe. A real, confirmed case had a tangent-length question whose scheme required TA/AM = OA/OM, while a student instead set θ = the angle at the centre and wrote tanθ = AC/OC (=5/12), then reapplied it as tanθ = AT/AO (=5/12) in the larger triangle -- algebraically the identical relationship, since cross-multiplying either version gives the same equation -- and reached the fully correct final value (AT = 65/12 cm), yet the step was marked not-met as though a wrong ratio had been used.

Before writing that a student's final numeric answer is wrong because it differs in FORM from the marking scheme's own stated value -- a rounded decimal (e.g. 5.42) versus an exact fraction (e.g. 65/12), or vice versa -- actually compute the true numeric value of both and compare them, the same digit-by-digit discipline already required for every other arithmetic check in this prompt. A real, confirmed case had a step's own feedback state both that a student's "AT=5.42cm" was wrong AND that "the correct [value is] 65/12" in the very same sentence, even though 65/12 = 5.41666..., which rounds to exactly 5.42 -- the two numbers were identical, and there was no error at all. Never let two representations of the same number be treated as if one contradicts the other.
...
${REVISION_SUBSCRIPT_INSTRUCTION}`;
    ...
    const gradeTool = {
      ...
      input_schema: {
        ...
        properties: {
                ...
                stepResults: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      note: { type: "string", description: "REQUIRED, never empty, written BEFORE met: silently work through whether the student's answer actually satisfies this specific step -- redo any arithmetic yourself digit by digit rather than eyeballing it, and resolve any doubt BEFORE writing anything here. This field must state ONLY your final, settled conclusion, never a narrated journey to get there -- do NOT write a 'first this looks wrong, but actually...' or 'X, which coincidentally/actually matches...' style sentence; if your own reasoning changed its mind partway through, throw away the abandoned first take entirely and write only the ending you settled on, as if that were the only thing you ever thought. If satisfied, write one short plain sentence confirming what they did right (e.g. 'Correctly simplified to 2√10, matches the required value'), not a sentence that also mentions an initial wrong-looking form. If not satisfied, one short SIMPLE sentence a student can immediately understand describing exactly what was wrong or missing. Write it exactly as you'd say it out loud standing next to the student, in second person ('you'), the way a friendly tutor points out a mistake -- never like an internal grading report. Never reference how grading works: don't write 'the marking scheme expects...', 'this step requires...', 'does not satisfy the criterion', 'concluding value unstated', or anything that assumes the student knows there's a scheme or steps at all -- a student has never seen it and doesn't think in those terms. Say plainly what they did (e.g. 'You didn't write the final answer for k', 'You added instead of multiplying here', 'You didn't mention the image is inverted') instead of describing them in the abstract passive voice. Critically, when what's missing is a specific fact, value, name, or conclusion, don't just say a version of it was missing -- SAY THE ACTUAL MISSING THING outright, in the same sentence, exactly like a teacher handing the answer back would. 'You didn't explicitly state which option gives more return' leaves a student who thinks they already answered it (e.g. they wrote the difference is ₹110.16) with no idea what else was even wanted; 'You worked out the ₹110.16 difference correctly, but never wrote the actual conclusion -- compounding gives the higher return, so that's the better option' tells them precisely what sentence they needed to add. The same goes for a missing name, term, or number: say what it actually was, don't just note that 'the name/term/value' was absent. This does NOT mean dumbing down the subject itself -- keep real syllabus vocabulary the student is actually taught and expected to use (coefficient, inversely proportional, photosynthesis, and so on); the only thing to strip out is grading/exam-report jargon that has nothing to do with the subject. Use real subscript/superscript characters for any chemical formula or exponent (e.g. N₂, x²), never a plain digit." },
          ...
          overallFeedback: { type: "string", description: "1-2 short, simple sentences a student can understand at a glance: strongest area and one concrete tip for next time. Written the same way as each step's note -- plain, second-person, like a tutor talking directly to the student, never grading-report language ('the scheme', 'criteria', 'assessment indicates'). The per-question marks and reasons are shown separately, so do not repeat question-specific detail here. NEVER write a sweeping claim like 'you didn't attempt questions X, Y, Z' or 'several questions weren't attempted at all' based on what this specific file happens to contain -- this may be an 'Improve Score' resubmission where the student deliberately only reworked their weaker questions and left their already-correct earlier answers in place (those are preserved and scored separately, from the earlier attempt, precisely so a student never has to redo a question they already got right); a blanket 'not attempted' claim here would be flatly false and confusing next to a score table that shows those very questions with full marks. If you can only see part of the full question set in front of you, simply don't comment on the parts you can't see at all, rather than assuming they were skipped. Use real subscript/superscript characters for any chemical formula or exponent (e.g. N₂, x²), never a plain digit." },
        },
  ...
  async function computeRevisionLeaderboard(classLabel: string) {
    ...
    for (const s of submissions || []) {
      ...
      // specifically because his one qualifying "improvement" was exactly this kind of accident. A
      // non-zero-but-still-wrong first score (a genuine grading misread, not a zero) is corrected by
      // nulling that submission's first_attempt fields directly rather than by a blanket rule here --
      // "several questions marked entirely unattempted" is the ordinary, expected shape of a real
      // first attempt for most students (they ran out of time, or hadn't reached later questions
      // yet), so treating that phrase itself as a bug signal would wrongly erase legitimate
      // first-attempt data for every normal Improve Score user, not just the rare misread (confirmed
      // for Sarbani, Class IX, by checking her case individually against her actual submission).
  ...
  app.post("/api/admin/revision/reference-books/finalize", async (req, res) => {
    ...
    if (isZip) {
      ...
      for (const entry of entries) {
        ...
        // Preserves spaces and apostrophes -- the "NN Chapter Title.pdf" naming convention every
        // other reference-book filename on file actually uses (referenceBookDisplayTitle strips
        // the "NN " prefix by matching whitespace, and chapterMatchScore matches on the real
        // title's own words) depends on the space surviving; sanitizing it away to an underscore
        // silently breaks both, and can zero out matching entirely for a short chapter name like
        // "Sets" once "01 Sets.pdf" becomes "01_Sets.pdf". Still blocks path separators and other
        // characters that could escape the intended storage folder.
        const safeName = baseName.replace(/[^a-zA-Z0-9.\-\s']/g, "_");
      ...
      const safeName = (fileName || "reference.pdf").replace(/[^a-zA-Z0-9.\-\s']/g, "_");
