...
import { LearnTissues9 } from './components/LearnTissues9';
...
import { LearnScience8Matter } from './components/LearnScience8Matter';
...
import {
  SCIENCE8IW_SOLVED_QUESTIONS,
  SCIENCE8IW_MCQS,
  SCIENCE8IW_VERY_SHORT,
  SCIENCE8IW_SHORT,
  SCIENCE8IW_LONG,
...
import {
  SCIENCE8M_SOLVED_QUESTIONS,
  SCIENCE8M_MCQS,
  SCIENCE8M_VERY_SHORT,
  SCIENCE8M_SHORT,
  SCIENCE8M_LONG,
  SCIENCE8M_COMPETENCY,
  SCIENCE8M_SELF_ASSESSMENT,
} from './data/science8Matter';
import {
  SCIENCE8EC_SOLVED_QUESTIONS,
  SCIENCE8EC_MCQS,
  SCIENCE8EC_VERY_SHORT,
  SCIENCE8EC_SHORT,
  SCIENCE8EC_LONG,
  SCIENCE8EC_COMPETENCY,
  SCIENCE8EC_SELF_ASSESSMENT,
} from './data/science8ElementsCompounds';
import {
...
import {
  ...
  CC10_LONG,
  CC10_COMPETENCY,
  CC10_SELF_ASSESSMENT,
} from './data/controlCoordination10';
import { REPRO10_MCQS } from './data/reproduction10_mcq';
...
import { FORCES9_MCQS } from './data/forces9_mcq';
import { FORCES9_VERY_SHORT, FORCES9_SHORT } from './data/forces9_short';
import { FORCES9_LONG, FORCES9_COMPETENCY } from './data/forces9_long';
import { FORCES9_NCERT_SOLVED, FORCES9_SELF_ASSESSMENT } from './data/forces9_practice';
import { ENERGY9_MCQS } from './data/energy9_mcq';
import { ENERGY9_VERY_SHORT, ENERGY9_SHORT } from './data/energy9_short';
import { ENERGY9_LONG, ENERGY9_COMPETENCY } from './data/energy9_long';
import { ENERGY9_NCERT_SOLVED, ENERGY9_SELF_ASSESSMENT } from './data/energy9_practice';
import { SOUND9_MCQS } from './data/sound9_mcq';
...
import { TISSUES9_MCQS } from './data/tissues9_mcq';
import { TISSUES9_VERY_SHORT, TISSUES9_SHORT } from './data/tissues9_short';
import { TISSUES9_LONG, TISSUES9_COMPETENCY } from './data/tissues9_long';
import { TISSUES9_NCERT_SOLVED, TISSUES9_SELF_ASSESSMENT } from './data/tissues9_practice';
import {
  Home,
  BookOpen,
  HelpCircle,
...
export default function App() {
  ...
  const [bioChapter9, setBioChapter9] = useState<'cell' | 'tissues'>('cell');
  ...
  const [scienceChapter8, setScienceChapter8] = useState<'investigative-world' | 'invisible-living-world' | 'health' | 'matter' | 'elements-compounds'>('investigative-world');
  ...
  const SUBJECT_AVAILABILITY: Record<'8th' | '9th' | '10th', Record<'Maths' | 'Science' | 'Physics' | 'Chemistry' | 'Biology', boolean>> = {
    '8th': { Maths: true, Science: true, Physics: true, Chemistry: false, Biology: false },
    '9th': { Maths: false, Science: false, Physics: true, Chemistry: false, Biology: true },
    '10th': { Maths: false, Science: false, Physics: true, Chemistry: true, Biology: true },
  };

  const openSubject = (cls: '8th' | '9th' | '10th', subject: 'Maths' | 'Science' | 'Physics' | 'Chemistry' | 'Biology') => {
    ...
    else if (subject === 'Science' && cls === '8th') { setQbSubject('science'); changeView('scienceNotes8'); }
  ...
  const SUBJECT_CARDS: { id: 'Maths' | 'Science' | 'Physics' | 'Chemistry' | 'Biology'; icon: React.ElementType; color: string }[] = [
    { id: 'Maths', icon: Calculator, color: 'from-violet-500/20 to-purple-600/20 border-violet-400 text-violet-400' },
    { id: 'Science', icon: Microscope, color: 'from-amber-500/20 to-orange-600/20 border-amber-400 text-amber-400' },
  ...
  const activeQuestions = useMemo(() => {
    if (preparingFor === '8th' && qbSubject === 'maths' && mathsChapter8 === 'square-and-cube') {
    ...
    } else if (preparingFor === '8th' && qbSubject === 'science' && scienceChapter8 === 'elements-compounds') {
      return {
        ncert: SCIENCE8EC_SOLVED_QUESTIONS,
        mcqs: SCIENCE8EC_MCQS,
        veryshort: SCIENCE8EC_VERY_SHORT,
        short: SCIENCE8EC_SHORT,
        long: SCIENCE8EC_LONG,
        assertion: [] as typeof MATHS8_ASSERTION_REASON,
        competency: SCIENCE8EC_COMPETENCY,
      };
    } else if (preparingFor === '8th' && qbSubject === 'science' && scienceChapter8 === 'matter') {
      return {
        ncert: SCIENCE8M_SOLVED_QUESTIONS,
        mcqs: SCIENCE8M_MCQS,
        veryshort: SCIENCE8M_VERY_SHORT,
        short: SCIENCE8M_SHORT,
        long: SCIENCE8M_LONG,
        assertion: [] as typeof MATHS8_ASSERTION_REASON,
        competency: SCIENCE8M_COMPETENCY,
      };
      ...
      return {
        ncert: SCIENCE8IW_SOLVED_QUESTIONS,
        mcqs: SCIENCE8IW_MCQS,
        veryshort: SCIENCE8IW_VERY_SHORT,
        short: SCIENCE8IW_SHORT,
        long: SCIENCE8IW_LONG,
        assertion: [] as typeof MATHS8_ASSERTION_REASON,
        competency: SCIENCE8IW_COMPETENCY,
      };
    } else if (preparingFor === '8th' && qbSubject === 'science') {
      return {
        ncert: SCIENCE8_SOLVED_QUESTIONS,
        mcqs: SCIENCE8_MCQS,
        veryshort: SCIENCE8_VERY_SHORT,
        short: SCIENCE8_SHORT,
        long: SCIENCE8_LONG,
        assertion: [] as typeof MATHS8_ASSERTION_REASON,
        competency: SCIENCE8_COMPETENCY,
      };
      ...
      return {
        ncert: REPRO10_NCERT_SOLVED,
        mcqs: REPRO10_MCQS,
        veryshort: REPRO10_VERY_SHORT,
        short: REPRO10_SHORT,
        long: REPRO10_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: REPRO10_COMPETENCY,
      };
    } else if (preparingFor === '10th' && qbSubject === 'biology' && bioChapter10 === 'control-coordination') {
      return {
        ncert: CC10_NCERT_SOLVED,
        mcqs: CC10_MCQS,
        veryshort: CC10_VERY_SHORT,
        short: CC10_SHORT,
        long: CC10_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: CC10_COMPETENCY,
      };
    ...
    } else if (preparingFor === '9th' && qbSubject === 'physics' && physicsChapter9 === 'sound') {
      return {
        ncert: SOUND9_NCERT_SOLVED,
        mcqs: SOUND9_MCQS,
        veryshort: SOUND9_VERY_SHORT,
        short: SOUND9_SHORT,
        long: SOUND9_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: SOUND9_COMPETENCY,
      };
    } else if (preparingFor === '9th' && qbSubject === 'physics' && physicsChapter9 === 'energy') {
      return {
        ncert: ENERGY9_NCERT_SOLVED,
        mcqs: ENERGY9_MCQS,
        veryshort: ENERGY9_VERY_SHORT,
        short: ENERGY9_SHORT,
        long: ENERGY9_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: ENERGY9_COMPETENCY,
      };
    } else if (preparingFor === '9th' && qbSubject === 'physics' && physicsChapter9 === 'forces') {
      return {
        ncert: FORCES9_NCERT_SOLVED,
        mcqs: FORCES9_MCQS,
        veryshort: FORCES9_VERY_SHORT,
        short: FORCES9_SHORT,
        long: FORCES9_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: FORCES9_COMPETENCY,
      };
    ...
    } else if (preparingFor === '9th' && qbSubject === 'biology' && bioChapter9 === 'tissues') {
      return {
        ncert: TISSUES9_NCERT_SOLVED,
        mcqs: TISSUES9_MCQS,
        veryshort: TISSUES9_VERY_SHORT,
        short: TISSUES9_SHORT,
        long: TISSUES9_LONG,
        assertion: [] as typeof CLASSX_ASSERTION_REASON,
        competency: TISSUES9_COMPETENCY,
      };
  ...
  }, [preparingFor, qbSubject, physicsChapter9, bioChapter9, bioChapter10, mathsChapter8, scienceChapter8]);
    ...
    <>
      ...
      {activeView === 'bioNotes' && (
        <div className="flex-1 flex flex-col overflow-hidden h-full">
          <div className={`shrink-0 flex items-center gap-2 px-4 py-2 border-b transition-colors duration-300 ${isLightMode ? 'bg-white border-slate-200' : 'bg-[#0d1424] border-slate-800'}`}>
            <span className="text-[11px] font-black uppercase tracking-widest font-mono mr-1 text-slate-500">Chapter:</span>
            {(['cell', 'tissues'] as const).map((ch) => (
              <button
                key={ch}
                onClick={() => setBioChapter9(ch)}
      ...
      {activeView === 'bioNotes10' && (
        <div className="flex-1 flex flex-col overflow-hidden h-full">
          <div className={`shrink-0 flex items-center gap-2 px-4 py-2 border-b transition-colors duration-300 ${isLightMode ? 'bg-white border-slate-200' : 'bg-[#0d1424] border-slate-800'}`}>
            ...
            <button
              onClick={() => setBioChapter10('life-processes')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                bioChapter10 === 'life-processes'
                  ? isLightMode ? 'bg-green-600 text-white' : 'bg-green-500 text-slate-950'
                  : isLightMode ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
            ...
            <button
              onClick={() => setBioChapter10('control-coordination')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                bioChapter10 === 'control-coordination'
                  ? isLightMode ? 'bg-green-600 text-white' : 'bg-green-500 text-slate-950'
                  : isLightMode ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
            ...
            <button
              onClick={() => setBioChapter10('reproduction')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
      ...
      {activeView === 'physicsNotes9' && (
        <div className="flex-1 flex flex-col overflow-hidden h-full">
          <div className={`shrink-0 flex items-center gap-2 px-4 py-2 border-b transition-colors duration-300 ${isLightMode ? 'bg-white border-slate-200' : 'bg-[#0d1424] border-slate-800'}`}>
            <span className="text-[11px] font-black uppercase tracking-widest font-mono mr-1 text-slate-500">Chapter:</span>
            {(['motion', 'forces', 'energy', 'sound'] as const).map((ch) => (
              <button
                key={ch}
                onClick={() => setPhysicsChapter9(ch)}
                className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                  physicsChapter9 === ch
              ...
              >
                {ch === 'motion' ? 'Motion' : ch === 'forces' ? 'Forces' : ch === 'energy' ? 'Work & Energy' : 'Sound'}
              </button>
            ))}
          </div>
          {physicsChapter9 === 'sound' ? (
            <LearnSound9
              isLightMode={isLightMode}
            ...
            />
          ) : physicsChapter9 === 'energy' ? (
            <LearnEnergy9
              isLightMode={isLightMode}
              onCompleteNotes={() => { setPreparingFor('9th'); setQbSubject('physics'); changeView('ncert'); }}
              onGoToSelfAssessment={() => { setPreparingFor('9th'); setQbSubject('physics'); changeView('assessment'); }}
            />
          ) : physicsChapter9 === 'forces' ? (
            <LearnForces9
              isLightMode={isLightMode}
              onCompleteNotes={() => { setPreparingFor('9th'); setQbSubject('physics'); changeView('ncert'); }}
              onGoToSelfAssessment={() => { setPreparingFor('9th'); setQbSubject('physics'); changeView('assessment'); }}
      ...
      {activeView === 'scienceNotes8' && (
        <div className="flex-1 flex flex-col overflow-hidden h-full">
          <div className={`shrink-0 flex items-center gap-2 px-4 py-2 border-b transition-colors duration-300 ${isLightMode ? 'bg-white border-slate-200' : 'bg-[#0d1424] border-slate-800'}`}>
            <span className={`text-[11px] font-black uppercase tracking-widest font-mono mr-1 ${isLightMode ? 'text-slate-500' : 'text-slate-500'}`}>Chapter:</span>
            <button
              onClick={() => setScienceChapter8('investigative-world')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                scienceChapter8 === 'investigative-world'
                  ? isLightMode ? 'bg-cyan-600 text-white' : 'bg-cyan-500 text-slate-950'
                  : isLightMode ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
            ...
            <button
              onClick={() => setScienceChapter8('invisible-living-world')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
            ...
            <button
              onClick={() => setScienceChapter8('matter')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                scienceChapter8 === 'matter'
                  ? isLightMode ? 'bg-cyan-600 text-white' : 'bg-cyan-500 text-slate-950'
                  : isLightMode ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
            ...
            <button
              onClick={() => setScienceChapter8('elements-compounds')}
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${
                scienceChapter8 === 'elements-compounds'
                  ? isLightMode ? 'bg-cyan-600 text-white' : 'bg-cyan-500 text-slate-950'
                  : isLightMode ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
          ...
          {scienceChapter8 === 'invisible-living-world' ? (
            ...
            <LearnScience8ElementsCompounds
              isLightMode={isLightMode}
              onCompleteNotes={() => { setPreparingFor('8th'); setQbSubject('science'); changeView('ncert'); }}
              onGoToSelfAssessment={() => { setPreparingFor('8th'); setQbSubject('science'); changeView('assessment'); }}
            />
      ...
      {activeView === 'ncert' && (
        ...
        >
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/40 border border-slate-800 p-6 rounded-2xl">
              <div className="space-y-1">
                <h2 className="text-xl font-black text-slate-100 tracking-tight flex items-center gap-2">
                  ...
                  {preparingFor === '9th' || (preparingFor === '8th' && (qbSubject === 'maths' || qbSubject === 'science')) || (preparingFor === '10th' && qbSubject === 'biology') ? 'Solved Textbook Questions' : 'Solved NCERT Questions'}
                </h2>
            ...
            {preparingFor === '9th' && qbSubject === 'physics' && (
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-black uppercase tracking-widest font-mono text-slate-500">Chapter:</span>
                <button onClick={() => setPhysicsChapter9('motion')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${physicsChapter9 === 'motion' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>Motion</button>
                <button onClick={() => setPhysicsChapter9('forces')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${physicsChapter9 === 'forces' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>Forces</button>
                <button onClick={() => setPhysicsChapter9('energy')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${physicsChapter9 === 'energy' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>Work & Energy</button>
                <button onClick={() => setPhysicsChapter9('sound')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${physicsChapter9 === 'sound' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>Sound</button>
              </div>
            )}
            ...
            {preparingFor === '10th' && qbSubject === 'biology' && (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setBioChapter10('life-processes')}
                  className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${bioChapter10 === 'life-processes' ? 'bg-green-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                >
                  Life Processes
                </button>
                <button
            ...
            {preparingFor === '8th' && qbSubject === 'science' && (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setScienceChapter8('investigative-world')}
                  className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${scienceChapter8 === 'investigative-world' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                >
                  Investigative World
                </button>
                <button
                ...
                <button
                  onClick={() => setScienceChapter8('matter')}
                  className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${scienceChapter8 === 'matter' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                >
                  Particulate Matter
                </button>
                <button
                  onClick={() => setScienceChapter8('elements-compounds')}
                  className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider transition cursor-pointer ${scienceChapter8 === 'elements-compounds' ? 'bg-cyan-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                >
                  Elements & Compounds
                </button>
              </div>
                  ...
                  <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden transition shadow-lg">
                    ...
                    {isOpen && (
                      <div className="p-6 border-t border-slate-800 bg-[#070d18] space-y-5">
                        ...
                        {/* Given values */}
                        <div className="space-y-2">
                          <span className="text-xs font-black text-slate-300 uppercase tracking-widest block font-mono">1. {(preparingFor === '9th' && qbSubject !== 'physics') || (preparingFor === '8th' && qbSubject === 'science') ? 'Key Concept / Context' : 'Extracted Values (Prior to substitution)'}</span>
                        ...
                        <div className="space-y-3 bg-slate-950/60 rounded-xl p-4.5 border border-slate-800">
                          ...
                          <div className="space-y-2 pt-2">
                            <span className="text-xs font-black text-slate-300 uppercase tracking-widest block font-mono">3. {(preparingFor === '9th' && qbSubject !== 'physics') || (preparingFor === '8th' && qbSubject === 'science') ? 'Step-by-Step Explanation' : 'Mathematical Progression Steps'}</span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                          <div className="bg-emerald-950/20 border border-emerald-500/25 rounded-xl p-4.5">
                            <span className="text-xs uppercase tracking-wider text-emerald-400 font-bold block font-mono mb-1">🎯 {(preparingFor === '9th' && qbSubject !== 'physics') || (preparingFor === '10th' && qbSubject === 'chemistry') || (preparingFor === '8th' && qbSubject === 'science') ? 'FINAL ANSWER' : 'FINAL NUMERICAL CONCLUSION'}</span>
                        ...
                        {!(preparingFor === '10th' && qbSubject === 'chemistry') && preparingFor !== '9th' && !(preparingFor === '8th' && qbSubject === 'science') && (
        ...
        >
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="max-w-2xl mx-auto">
              <Quiz
                key={`${preparingFor}_${qbSubject}_${bioChapter10}_${mathsChapter8}_${scienceChapter8}`}
                ...
                customQuestions={
                  preparingFor === '8th' && qbSubject === 'maths' && mathsChapter8 === 'square-and-cube'
                    ...
                    ? SCIENCE8H_SELF_ASSESSMENT
                    : preparingFor === '8th' && qbSubject === 'science' && scienceChapter8 === 'matter'
                    ? SCIENCE8M_SELF_ASSESSMENT
                    : preparingFor === '8th' && qbSubject === 'science' && scienceChapter8 === 'elements-compounds'
                    ? SCIENCE8EC_SELF_ASSESSMENT
                    : preparingFor === '8th' && qbSubject === 'science'
                    ? SCIENCE8_SELF_ASSESSMENT
                    : preparingFor === '8th'
                    ...
                    : preparingFor === '9th'
                    ? (qbSubject === 'physics' ? (physicsChapter9 === 'sound' ? SOUND9_SELF_ASSESSMENT : physicsChapter9 === 'energy' ? ENERGY9_SELF_ASSESSMENT : physicsChapter9 === 'forces' ? FORCES9_SELF_ASSESSMENT : PHYSICS9_SELF_ASSESSMENT) : qbSubject === 'foundations' ? FOUNDATIONS9_SELF_ASSESSMENT : bioChapter9 === 'tissues' ? TISSUES9_SELF_ASSESSMENT : BIOLOGY9_SELF_ASSESSMENT)
                    : preparingFor === '10th'
